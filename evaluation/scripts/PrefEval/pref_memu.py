...
def add_memory_for_line(
    line_data: tuple, mem_client, num_irrelevant_turns: int, lib: str, version: str, success_records, f
) -> dict:
    """
    ...
    try:
        ...
        for idx, _ in enumerate(conversation[::2]):
            ...
            if record_id not in success_records:
                mem_client.add(
                        messages=conversation[msg_idx : msg_idx + 2], user_id=user_id, iso_date=datetime.now().isoformat()
                    )
                f.write(f"{record_id}\n")
                f.flush()
